...
if __name__ == '__main__':
    ...
    train_data, val_data = build_wiki_lingual(part_name='English2%s' % appoint_lingual)

    save_path = 'WikiLingualResult-MultiTask-English2%s/' % appoint_lingual
    ...
    for index, sample in enumerate(val_data):
        ...
        article = sample['%sDocument' % appoint_lingual]
        cross_lingual_summary = sample['EnglishSummary']
        article_tokenized = tokenizer.encode(
            'summarize %s to English : ' % appoint_lingual + article, return_tensors='pt', max_length=512)
